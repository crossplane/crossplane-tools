...
func (t *Traverser) Traverse(n *types.Named, cfg *ProcessorConfig, formerFields ...string) error { // nolint:gocyclo
	...
	for i := 0; i < st.NumFields(); i++ {
		field := st.Field(i)
		tag := st.Tag(i)
		if err := cfg.Field.Process(n, field, tag, t.comments.For(field), formerFields); err != nil {
